...
/**
 * IMPORTANT
 * Do not edit this file directly in the folder /web/sites/default/ as it will
 * be overriden by the scaffolding file located in /config/scaffold/sites/default/
 * on each build.
 */
...
// Handle Drush problem when using MAMP => https://modulesunraveled.com/blog/drush-not-working-mamp-heres-how-fix-it.
if (getenv('DB_UNIX_SOCKET')) {
  $databases['default']['default']['unix_socket'] = getenv('DB_UNIX_SOCKET');
}
...
if ($sentry_dsn = getenv('SENTRY_DSN')) {
  $sentry_config = [
    'dsn' => $sentry_dsn,
    'environment' => (getenv('SENTRY_ENV')) ?: getenv('APP_ENV'),
  ];
...
$ddev_settings = $base_path . '/settings.ddev.php';
if (getenv('IS_DDEV_PROJECT') == 'true' && is_readable($ddev_settings)) {
  require $ddev_settings;
  $databases['default']['default']['prefix'] = getenv('DB_PREFIX');
}

// Handle ELK configuration.
$settings['elasticsearch_index'] = getenv('ELASTICSEARCH_INDEX');

#
if (getenv('APP_DEBUG') === 'true' && file_exists($app_root . '/' . $site_path . '/settings.local.php')) {
